...
if(length(args)<4) {
  ...
  nsam <- 100
  niter <- 100
  path_file <- "C:/Users/oSCAR/Documents/RsTUDIO/Populations"
} else {
...
for(fil in c(1:4)) {
  ...
  plot(density(data.results.A$ThetaAWatt),main=sprintf("Model %s \n Theta from SFS total",model[fil]),xlim=c(0,max.val),ylim=c(0,2500)); abline(v=mean(data.results.A$ThetaAWatt,na.rm=T))
  ...
  plot(density(data.results.Watt[,-c(1:3)]),main=sprintf("Model %s \n Theta from relative SFS",model[fil]),xlim=c(0,max.val),ylim=c(0,2500)); abline(v=mean(data.results.Watt[,-c(1:4)],na.rm=T))
  ...
  for(i in c(1:4)) { # AA.AA tests. Collect only one value per iteration (all ind. have the same value)
    plot(density(coord.test.matrix[seq(1,niter*npops,niter),i]),main=sprintf("Model %s \n Test sd %s",model[fil],name.test[i]))
    abline(v=mean(coord.test.matrix[seq(1,niter*npops,niter),i]))
    if(fil > 1) {
      lines(density(coord.test.matrix1[seq(1,niter*npops,niter),i]),col="grey")
      abline(v=mean(coord.test.matrix1[seq(1,niter*npops,niter),i]),col="grey")
  ...
  for(i in c(5:8,5,9:15)) { # one plot (5) is repeated.
    ...
    if(fil > 1) {
        lines(density(coord.test.matrix1[1:(100/popsize),i]),col="grey")
        abline(v=mean(coord.test.matrix1[1:(100/popsize),i]),col="grey")
        #legend("topright",legend=c("SNM"),lty=1,col="grey")
    }
  }
}
