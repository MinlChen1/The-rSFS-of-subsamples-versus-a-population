...
model <- c("SNM","Complete Sweep","Incomplete Sweep","Soft Sweep")
...
if(popsize==2) data.namesPI  <- c("slim.snm.output_file.ms_mstatspopo1PI.txt","slim.selsweep.output_file.ms_mstatspopo1PI.txt","slim.incomplete_selsweep.output_file.ms_mstatspopo1PI.txt","slim.standing_selsweep.output_file.ms_mstatspopo1PI.txt")
...
for(fil in c(1:4)) {
  ...
  #ESTIMATE thetaTaj
  columns.PI  <- c(c(paste0("ThetaTaj", 0:49)))
  data.results.PI  <- data.frame(matrix(nrow=0,ncol=length(columns.PI)))
  colnames(data.results.PI) <- columns.PI
  
  data.freqsPI <- read.table(file=sprintf("%s/%s.columns.txt",path_file,data.namesPI[fil]),header=T)
  
  init.pipi.values <- grep("Theta.Taj.",colnames(data.freqsPI))
  
  p.len.seqPI <- grep("Eff_length2_pop",colnames(data.freqsPI))[1]#take only one. All are equal under option -u 0
  num.genePI <- 1
  for(num.genePI in 1:dim(data.freqsPI)[1]) {
    Theta.PI.line <- data.freqsPI[num.genePI,init.pipi.values]/data.freqsPI[num.gene,p.len.seqPI]
    data.results.PI  <- rbind(data.results.PI,data.frame(Theta.PI.line)) #,data.freqs[num.gene,p.len.seq],Theta.A.line[1],Theta.A.line[2],Theta.A.line[3],Theta.A.line[4]))
  }
  colnames(data.results.PI)  <- columns.PI
  data.results.PI  <- data.results.PI[!duplicated(data.results.PI),]
  write.table(file=sprintf("%s/Results_PIDiversity_unfold.txt",path_file),x=data.results.PI, quote=F,row.names=F)
  ...
  plot(density(data.results.A$ThetaAWatt),main=sprintf("Model %s \n Theta from SFS",model[fil]),xlim=c(0,max.val),ylim=c(0,2500)); abline(v=mean(data.results.A$ThetaAWatt,na.rm=T))
  ...
  plot(density(data.results.Watt[,-c(1:3)]),main=sprintf("Model %s \n Theta from rSFS",model[fil]),xlim=c(0,max.val),ylim=c(0,2500)); abline(v=mean(data.results.Watt[,-c(1:3)],na.rm=T))
  lines(density(data.results.Fuli[,-c(1:3)]),col="blue"); abline(v=mean(data.results.Fuli[,-c(1:3)],na.rm=T),col="blue")
  lines(density(data.results.Taj[,-c(1:3)]),col="red"); abline(v=mean(data.results.Taj[,-c(1:3)],na.rm=T),col="red")
  lines(density(data.results.fw[,-c(1:3)]),col="green"); abline(v=mean(data.results.fw[,-c(1:3)],na.rm=T),col="green")
  legend("topright",legend=c("Watt","FuLi","Taj","FayWu"),lty = 1,col=c("black","blue","red","green"))
  
  #Plot the distribution of r_theta values per iteration
  colors <- rainbow(10) 
  par(mfrow=c(2,2))
  plot(density(data.results.Fuli[1,c(4:(4+nsam/popsize-1))]),xlim=c(0,2e-2),ylim=c(0,4e2),main=sprintf("%s\nrFu&Li ind. Distribution per iteration",model[fil]), col = colors[1])
  for(i in 2:10) {
    lines(density(data.results.Fuli[(i*10-9),c(4:(4+nsam/popsize-1))]), col = colors[i]);
  }
  plot(density(data.results.Watt[1,c(4:(4+nsam/popsize-1))]),xlim=c(0,1e-2),ylim=c(0,1e3),main=sprintf("%s\nrWatterson ind. Distribution per iteration",model[fil]), col = colors[1])  
  for(i in 2:10) {
    lines(density(data.results.Watt[(i*10-9),c(4:(4+nsam/popsize-1))]), col = colors[i]);
  }
  plot(density(data.results.Taj[1,c(4:(4+nsam/popsize-1))]),xlim=c(0,5e-3),ylim=c(0,3e3),main=sprintf("%s\nrTajima ind. Distribution per iteration",model[fil]), col = colors[1])  
  for(i in 2:10) {
    lines(density(data.results.Taj[(i*10-9),c(4:(4+nsam/popsize-1))]), col = colors[i]);
  }
  plot(density(data.results.fw[1,c(4:(4+nsam/popsize-1))]),xlim=c(0,5e-3),ylim=c(0,4e3),main=sprintf("%s\nrFay&Wu ind. Distribution per iteration",model[fil]), col = colors[1])  
  for(i in 2:10) {
    lines(density(data.results.fw[(i*10-9),c(4:(4+nsam/popsize-1))]), col = colors[i]);
  }
  ...
  name.test <- c("Taj.Watt","Taj.FuLi","Taj.Faywu", "Watt.FayWu",
                 "Watt.RWatt","FuLi.RFuLi","Taj.RTaj", "FayWu.RFayWu",
                 "Watt.RFuLi","Watt.RTaj","Watt.RFayWu",
  ...
  par(mfrow=c(2,2))
  for(i in c(1:4)) { # AA.AA tests. Collect only one value per iteration (all ind. have the same value)
    plot(density(coord.test.matrix[seq(1,niter*npops,npops),i]),xlim = c (-14,3), main=sprintf("Model %s \n Test sd %s",model[fil],name.test[i]))
    abline(v=mean(coord.test.matrix[seq(1,niter*npops, npops),i]))
    if(fil > 1) {
      lines(density(coord.test.matrix1[seq(1,niter*npops, npops),i]),col="red")
      abline(v=mean(coord.test.matrix1[seq(1,niter*npops, npops),i]),col="red")
      legend("topright",legend=c("SNM"),lty=1,col="red")
    }
  }
  for(i in c(5:8,5,9:15)) { # one plot (5) is repeated.
    plot(density(coord.test.matrix[,i]),main=sprintf("Model %s \n Test sd %s",model[fil],name.test[i]))
    abline(v=mean(coord.test.matrix[,i]))
    if(fil > 1) {
      lines(density(coord.test.matrix1[,i]),col="red")
      abline(v=mean(coord.test.matrix1[,i]),col="red")
      legend("topright",legend=c("SNM"),lty=1,col="red")
    }
    plot(density(coord.test.matrix[1:(100/popsize),i]),main=sprintf("Model %s \n Test sd %s single",model[fil],name.test[i]))
    abline(v=mean(coord.test.matrix[1:(100/popsize),i]))
    if(fil > 1) {
        lines(density(coord.test.matrix1[1:(100/popsize),i]),col="red")
        abline(v=mean(coord.test.matrix1[1:(100/popsize),i]),col="red")
        legend("topright",legend=c("SNM"),lty=1,col="red")
    }
  }
  
  data.results.PI.melt <- melt(data.results.PI)
  plot(density(data.results.PI.melt[,2]),main=sprintf("Model %s \n Heterozygosity",model[fil]))
  abline(v=mean(data.results.PI.melt[,2]))
  plot(density(as.numeric(data.results.PI[1,])),main=sprintf("Model %s \n Heterozygosity single",model[fil]))
  abline(v=mean(as.numeric(data.results.PI[1,])))
}
######################################################################
dev.off()
